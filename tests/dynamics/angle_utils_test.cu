...
TEST(AngleUtils, normalizeAngleKnownFloat)
{
  ...
  angle = M_PI_2;
  EXPECT_FLOAT_EQ(angle_utils::normalizeAngle(angle), M_PI_2f);

  angle = M_PI;
  EXPECT_FLOAT_EQ(angle_utils::normalizeAngle(angle), M_PIf);

  angle = -M_PI;
  EXPECT_FLOAT_EQ(angle_utils::normalizeAngle(angle), M_PIf);

  angle = -M_PIf * 3 + 1e-3;
  EXPECT_FLOAT_EQ(angle_utils::normalizeAngle(angle), -M_PIf + 1e-3);
